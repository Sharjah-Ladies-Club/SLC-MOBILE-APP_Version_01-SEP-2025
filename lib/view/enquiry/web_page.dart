...
class EnquiryWebviewPage extends StatefulWidget {
  EnquiryWebviewPage(
      {Key key,
      this.url,
      this.title,
  ...
  _EnquiryWebviewPageState createState() => _EnquiryWebviewPageState();
}

class _EnquiryWebviewPageState extends State<EnquiryWebviewPage> {
  ...
  void initState() {
    ...
    _onStateChanged = flutterWebViewPlugin.onStateChanged.listen((
      ...
      if (state.url.contains("http://192.168.1.34/slcapi/api/facilities/enquiry-payment-result") ||
          state.url.contains(
