...
class FacilityBuyBloc extends Bloc<FacilityBuyEvent, FacilityBuyState> {
  final FacilityBuyBloc fitnessBuyBloc;

  FacilityBuyBloc({@required this.fitnessBuyBloc, facilityBuyBloc})
      : super(null);

  FacilityBuyState get initialState => InitialFitnessBuyState();

  Stream<FacilityBuyState> mapEventToState(
    FacilityBuyEvent event,
  ) async* {
    if (event is GetItemDetailsEvent) {
      ...
      Meta m = await FacilityDetailRepository()
          .getKunoozOrderStatus(event.merchantReferenceNo);
      if (m.statusCode == 200) {
